...
The host owns its work items, governance, workflow trigger and results. This repository owns setup, invocation, claims and execution. The default setup installs a checked local copy. A host may instead pin this repository as a submodule and call its `invoke-dispatch.ps1`; executable files must remain clean in that checkout.
